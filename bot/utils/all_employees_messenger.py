...
async def all_employees_messenger(files_id, caption, bot, loger):
    ...
    for employer_id in employers:
        try:
            ...
            if not chat.id:
                loger.error(f"Бот не может отправлять сообщения в чат {employer_id}.")
                continue
            ...
            loger.error(f"Произошла ошибка при отправке в чат {employer_id}: {e}")